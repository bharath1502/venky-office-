...
public class DateUtils {
	...
	public static Timestamp getDailyFrequency(Date today) {
		...
		String date = strTodays.substring(Constants.EIGHT, Constants.MAX_ENTITY_DISPLAY_SIZE);
		Date convertedDateFormat = null;
		try {
			convertedDateFormat = formatter.parse(year + "-" + month + "-" + date + " " + "00:00:00");
			if(null != convertedDateFormat){
	...
	public static boolean isEligOrderPrinting(String orderReqDateTime) {
		...
		long day = Constants.DAY_TO_MILLISECONDS;
		Timestamp hisDate = new Timestamp(new Timestamp(System.currentTimeMillis()).getTime() - day);
		...
		try {
			...
			try {
				...
				logger.error("ERROR:: DateUtils::isEligOrderPrinting ", exp);
				isOrderPrinting = false;
			}
		}
	...
	public static Timestamp getTimestamp(String date) {
		try {
			...
			logger.error("ERROR:: DateUtils::getDateOfDaysBeforeCurrentDateAndTime ", e);
		}
		return null;
	}
	...
	public static boolean validateStEndDate(String startDate, String endDate) {
		...
		if(startDate == null || Constants.EMPTY_STRING.equals(startDate) || startDate.length() != Constants.EIGHT) {
			validFlag = false;
		}
		try {
			if(validFlag) {
				...
				currentDate = getSqlDateFromDateStr(getDateStrFromSqlTimestatmp(currentDateTime));
					Timestamp timestamp = getSqlDateFromDateStr(startDate);
	...
	private static boolean validateEndDate(String startDate, String endDate , Timestamp currentDate) {
		boolean validFlag = false;
		if(endDate != null && !Constants.EMPTY_STRING.equals(endDate)) {
			...
			if(endDate.length() != Constants.EIGHT) {
				...
				validFlag = validateEndingYear(startDate, endDate , currentDate, endingYear);
			}
	...
	private static boolean validateEndingYear(String startDate, String endDate,
			Timestamp currentDate, int endingYear) {
		boolean validFlag = false;
		...
		else if(timestampval != null && !timestampval.before(getSqlDateFromDateStr(formatTranDate(endDate)))) {
			validFlag = false;
		}
		return validFlag;
	}
	...
	public static String formatTranDate(String inputDate) {
		...
		try {
			...
			logger.error("ERROR:: DateUtils::formatTranDate ", exp);
		}
	...
	public static boolean validateTranMonth(String tranMonth) {
		...
		if(null == tranMonth || tranMonth.length() != Constants.FOUR) {
			validFlag = false;
		}
	...
	public static String getFormatDate(Timestamp dateWithTime) {
		String strTodays = dateWithTime.toString();
		return strTodays.substring(0, Constants.FOUR) + strTodays.substring(Constants.FIVE, Constants.SEVEN) + strTodays.substring(Constants.EIGHT, Constants.MAX_ENTITY_DISPLAY_SIZE)
	...
	public static String getFormattedTime() {
		...
		long monthVal = month;
		formattedTime.append(formatter.format(monthVal));
