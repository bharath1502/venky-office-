		...
		<div class="container-fluid">
			...
			<article>
				<div class="col-xs-12 content-wrapper">
					...
					<c:if test="${not empty rolesCreate && fn:contains(existingFeatures,rolesCreate)}">
											...
											<fieldset class="col-md-3 col-sm-6"> 
												...
												<fieldset class="col-md-3 col-sm-6">
													...
													<form:input cssClass="form-control" id="roleName"
														 path="roleName"
														 onblur="this.value=this.value.trim();validRoleName()" maxlength="30" />
													<div class="discriptionErrorMsg" data-toggle="tooltip" data-placement="top" title="">
														<span class="red-error" id="roleNameEr">&nbsp;</span>
													</div>
												</fieldset>
												<fieldset class="col-md-3 col-sm-6">
																	...
																	<div class="panel panel-default clearfix create-dropdown " >
																		...
																		<div class="panel-heading permission_color" >
																			<h4 class="panel-title">
																				<a id="anchorClick${submitDatasubmitData.getFeatureId()}"
																					data-toggle="collapse" data-parent="#accordion"
																					href="#collapse${feature.getFeatureId()}"  class="fa fa-angle-double-left"  ><label class="permission_cursor_type" style="font-size: 14px;font-family: Verdana, Geneva, sans-serif;font-style: normal;">
																		...
																		<div id="collapse${feature.getFeatureId()}"
																			...
																			<li><label data-toggle="tooltip" data-placement="top" title=""><input type="checkbox" name="featureTO" value="${feature.getFeatureId()}"
																					id="${feature.getFeatureId()}" <c:if test="${feature.getName() eq 'Dashboard'}">disabled checked</c:if> />
																					<c:if test="${feature.getName() eq 'Dashboard'}"><spring:message code="header.label.dashboard"/></c:if>
																					<c:if test="${feature.getName() eq 'SETUP'}"><spring:message code="header.label.setup"/></c:if>
																					<c:if test="${feature.getName() eq 'Programs'}"><spring:message code="commission-program-search.label.programs"/></c:if>
																					<c:if test="${feature.getName() eq 'Manage'}"><spring:message code="manage.label.manage1"/></c:if>
																				...
																				<ul>
																					...
																					<c:forEach items="${featureList}" var="featureSub" varStatus="counter">
																						<c:if test="${featureSub.getRefFeatureId() eq feature.getFeatureId()}">
																							...
																							<!-- DisplayChild(Level2) -->
																							<fieldset class="col-md-3 col-sm-6">
																							<li><label data-toggle="tooltip" data-placement="top" title=""><input type="checkbox" name="featureTO" id="${featureSub.getFeatureId()}" <c:if test="${feature.getName() eq 'Dashboard'}">disabled checked</c:if> />
																								...
																								<ul>
																									...
																									<c:forEach items="${featureList}" var="featureChild">
																										<c:if test="${featureChild.getRefFeatureId() eq featureSub.getFeatureId()}">
																											<!-- DisplayChild(Level3) -->
																											<li><label data-toggle="tooltip" data-placement="top" title=""><input type="checkbox" name="featureTO" id="${featureChild.getFeatureId()}" <c:if test="${feature.getName() eq 'Dashboard'}"> checked</c:if> />
                                               ...
                                               <!--Panel Action Button Start -->
													<div class="col-sm-12 form-action-buttons">
														...
														<div class="col-sm-7">
															<input type="submit"
															  id="buttonCreate"	class="form-control button pull-right" value='<spring:message code="common.label.create"/>'
															...
															<a href="access-role-create"
																class="form-control button pull-right"><spring:message code="common.label.reset"/></a>
														</div>
													</div>
