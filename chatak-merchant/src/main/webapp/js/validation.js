...
function clientValidation(field_id, field_name, div_id) {
	...
	if (ValidationRules[name].type == "alphanumsplchar") {
		if (!alphanumsplchar(data, div_id))
			return false;
	}

	if (ValidationRules[name].type == "amount") {
		if (!amount(data, div_id)) {
			errorFieldFocus(field_id);
			return false;
		}
	...
	if(getUserLocale() != 'es'){
	if (ValidationRules[name].type == "dollarnumeric") {
		if (!dollarnumeric(data, div_id))
			return false;
	}
	}
	...
	if (ValidationRules[name].type == "startDate") {
		if (!validateFromDate(data, div_id))
			return false;
	}

	if (ValidationRules[name].type == "endDate") {
		if (!validateToDate(data, div_id))
			return false;
	}
...
function alpha(data, div_id) {
	...
	if (regex.test(data)) {
		...
		setDiv(div_id, webMessages.shouldcontainonlyalphabets);
		loadMsgTitleText();
		return false;
	}
}
...
function numericWithDash(data, div_id) {
	...
	if (regex.test(data)) {
		...
		setDiv(div_id, webMessages.Shouldcontainonlynumericanddash);
		loadMsgTitleText();
		return false;
	}
}
...
function mandatorytrue(data, div_id) {
	if (data == "") {
		setDiv(div_id, webMessages.thisfieldismandatory);
		loadMsgTitleText();
		return false;
	} else {
		setDiv(div_id, "");
...
function minimum(div_id) {
	...
	setDiv(div_id, webMessages.invalidlength);
	loadMsgTitleText();
	return false;
}

function maximum(div_id) {
	setDiv(div_id, webMessages.exceedinglength);
	loadMsgTitleText();
	return false;
}
...
function numeric(data, div_id) {
	...
	if (regex.test(data)) {
		...
		setDiv(div_id, webMessages.shouldcontainonlynumbers);
		loadMsgTitleText();
		return false;
	}
}

function dollarnumeric(data, div_id) {
	var regex = /^(\d{1,3}(\,\d{3})*|(\d+))(\.\d{2})?$/;
	if (regex.test(data)) {
		setDiv(div_id, "");
		return true;
	} else {
		setDiv(div_id, webMessages.improperFormatEx);
		return false;
	}
}
...
function mobile(data, div_id) {
	...
	if ($("#mobileNumber").val().replace(/_/g, "").length == 2) {
		setDiv(div_id, webMessages.thisfieldismandatory);
		loadMsgTitleText();
		return false;
	}
	...
	if (regex.test(data)) {
		...
		setDiv(div_id, webMessages.shouldnotstartwithzero);
		loadMsgTitleText();
		return false;
	}
}
...
function numericNotZero(data, div_id) {
	...
	if (regex.test(data)) {
		...
		setDiv(div_id, webMessages.shouldcontainonlypositivenumbers);
		loadMsgTitleText();
		return false;
	}
}
...
function alphaNumeric(data, div_id) {
	...
	if (regex.test(data)) {
		...
		setDiv(div_id, webMessages.cancontainonlyalphabetsandnumerics);
		loadMsgTitleText();
		return false;
	}
}
...
function password(data, div_id) {
	...
	if (regex.test(data)) {
		...
		setDiv(div_id,webMessages.shouldcontainmin1Caps1specialcharand1Number	);
		loadMsgTitleText();
		return false;
	}
}
...
function createBinValidation() {
	...
	if (!clientValidation('bin', 'bin', 'binerrormsg')
			...
			| !clientValidation('partnerName', 'bin_partner_name',
					'partnernameerrormsg') | !validateEffectiveDate()) {
		flag = false;
		return flag;
	}
	return flag;
...
function validateEffectiveDate() {
	...
	if (selectedEffDT == '' || selectedEffDT.length == 0) {
		setDiv("dateerrormsg", webMessages.thisfieldismandatory);
		loadMsgTitleText();
		return false;
	} else if (Date.parse(currentEffDT) > Date.parse(selectedEffDT)) {
		setDiv("dateerrormsg", webMessages.effectivedateshouldnotbeolderdate);
		loadMsgTitleText();
		return false;
	} else {
		setDiv("dateerrormsg", " ");
...
function readPartnerLogo(input, div_id) {
	...
	if (!isValidImage(input.value)) {
		document.getElementById(div_id).innerHTML = message.invalidImageFormat;
		return false;
	}
...
function numericDecimal(data, div_id) {
	...
	if (regex.test(data)) {
		...
		setDiv(div_id, webMessages.enterpropervalue);
		loadMsgTitleText();
		return false;
	}
}
...
function validateFromDate(divId, errorMsg) {
	...
	if (!eairlyDate(fromDate)) {
		setDiv(errorMsg, webMessages.startdatecannotbelesserthenpresentdate);
		loadMsgTitleText();
		flag = false;
		return flag;
	} else {
...
function validateToDate(divId, errorMsg) {
	...
	if (!eairlyDate(toDate)) {
		setDiv(errorMsg,webMessages.enddateshouldbegreaterthenpresentdate);
		loadMsgTitleText();
		flag = false;
		return flag;
	} else {
...
function validateDates(divId, errorMsg) {
	...
	if (toDate != "") {
		if (newFromDate > newToDate) {
			flag = false;
			setDiv(errorMsg, webMessages.startdatecannotbegreaterthanEnddate);
			loadMsgTitleText();
		} else {
			setDiv(errorMsg, "&nbsp;");
		}
...
function eairlyDate(idate) {
	...
	var newMonth = parseInt(idate.split("/")[0]);
	var newDay = parseInt(idate.split("/")[1]);
	var newYear = parseInt(idate.split("/")[2]);
...
function validateRadio(err) {
	if (get('No').checked == false && get('Yes').checked == false) {
...
function amount(data, div_id) {
	...
	if (regex.test(data)) {
		...
		setDiv(div_id, webMessages.invalidamount);
		loadMsgTitleText();
		return false;
...
function alphanumsplchar(data, div_id) {
	var regex = /^[A-Za-z0-9@][A-Za-z0-9!#$%^&*'()+\=\[\]{};:"\\|,.<>\/? ]*$/;
	if (regex.test(data)) {
		setDiv(div_id, "");
		return true;
	} else {
		setDiv(div_id, webMessages.shouldnotstartwithspecialchar);
		loadMsgTitleText();
		return false;
	}
}

function validateMerchantRadio(err) {
	if (get('merchant').checked == false && get('subMerchant').checked == false) {
		setDiv(err, webMessages.pleaseselectone);
		loadMsgTitleText();
		return false;
	} else {
