...
<body oncontextmenu="disableRightClick(<%=StatusConstants.ALLOW_RIGHT_CLICK%>)">
			...
			<article>
				<div class="col-xs-12 content-wrapper">
					...
					<div class="tab-header-container-first active-background">
						<a href="#"><spring:message code="dash-board.label.completedtransactions"/></a>
					</div>
					<!-- Content Block Start -->
					...
					<div class="col-xs-12">
						...
						<div class="search-results-table">
							...
							<tr>
								<td colspan="6" class="search-table-header-column " style="text-align: left">
									<spring:message code="dash-board.label.transactionsummary"/>
						...
						<table id="serviceResults" class="table table-striped table-bordered table-responsive table-condensed tablesorter">
							<thead>
								<tr>
									<th style="width: 100px;"><spring:message code="dash-board.label.transactiontime"/></th>
									<th style="width: 100px;"><spring:message code="merchant.common-deviceLocalTxnTime"/></th>
									<th style="width: 100px;"><spring:message code="dash-board.label.processedtime"/></th>
									<th style="width: 100px;"><spring:message code="dash-board.label.accounttransactionid"/></th>
									<th style="width: 100px;"><spring:message code="dash-board.label.transactionid"/></th>
									<th style="width: 50px;"><spring:message code="search-sub-merchant.label.currencycode"/></th>
									<th style="width: 50px;"><spring:message code="dash-board.label.type"/></th>
									<th style="width: 400px;"><spring:message code="dash-board.label.description"/></th>
									<th style="width: 50px;"><spring:message code="dash-board.label.debit"/></th>
									<th style="width: 60px;"><spring:message code="dash-board.label.credit"/></th>
									<th style="width: 60px;"><spring:message code="dash-board.label.currentbalance"/></th>
									<th style="width: 50px;"><spring:message code="dash-board.label.status"/></th>
								</tr>
							</thead>
							<tbody>
								<c:choose>
										...
										<c:forEach items="${executedTxnList}" var="completedTxns">
											<tr data-txn-merchant-code="${completedTxns.merchantCode}">
												<td class="tbl-text-align-left">${completedTxns.transactionTime}</td>
													<td class="tbl-text-align-left">${completedTxns.deviceLocalTxnTime}<c:if
									...
									<c:otherwise>
										<tr><td colspan="9" style="color: red; text-align: center;"><spring:message code="reportsglobal.label.norecordsfound"/></td></tr>
									</c:otherwise>
								</c:choose>
							</tbody>
						</table>
	...
	<div id="voidPopup" class="popup-void-refund void">
		<div class="popup-hf-bc fw-b-fs15 txt-center"><span style="line-height: 2em;"><spring:message code="dash-board.label.voidpayment"/></span></div>
		<div class="fw-b-fs15" style="padding: 20px;"><spring:message code="dash-board.label.areyousureyouwanttovoidthispayment"/>?</div>
		<div class="fw-b-fs15" style="padding: 0px 0px 20px 20px; font-style: italic; color: #bfbfbf;"><spring:message code="dash-board.label.thiscanNOTbeundone"/></div>
		<div class="col-sm-12 popup-hf-bc">
			<input type="submit" class="form-control button pull-right margin5" value="Void" onclick="voidTxn();"> 
			<input type="button" class="form-control button pull-right margin5 close-btn" value="Cancel" >
		</div>
	...
	<div id="voidResultPopup" class="popup-void-refund voidResult">
		...
		<div class="col-xs-12" id="voidResultInfo">
			<div class="fw-b-fs15"><spring:message code="dash-board.label.pleasenotedownthebelowinformation"/>:</div>
			<br>
			<div>
				<label data-toggle="tooltip" data-placement="top" title=""><spring:message code="dash-board.label.transactionreferenceno"/>. : </label><span id="txnRefNo">&nbsp;</span> 
			</div>
			<br>
		</div>
	...
	<div id="refundResultPopup" class="popup-void-refund voidResult">
		...
		<div class="col-xs-12" id="refundResultInfo">
			...
			<div>
				...
				<label data-toggle="tooltip" data-placement="top" title=""><spring:message code="dash-board.label.transactionreferenceno"/>. : </label><span id="refundTxnRefNo">&nbsp;</span> 
				<label data-toggle="tooltip" data-placement="top" title=""><spring:message code="dash-board.label.CGreferenceno"/>. : </label><span id="cgRefNo">&nbsp;</span>
			</div>
			<br>
		</div>
	...
	$(document).ready(function() {
		...
		$('.txn-action').on('click', function() {
			if($(this).attr('data-txn-status') == 'Processing') {
				...
				$('#refundMerchantId').val($(this).parent().attr('data-txn-merchant-code'));
				$('#refundAccountTransactionId').val($(this).siblings().find('.t-txn-id').text());
				$('#refundTotalTxnAmount').val($(this).siblings('.credit').text()	);
				getRefundBalance();
				if($('#partialRefund').is(':checked')) {
					$('#partialRefund').removeAttr('checked');
					$('#fullRefund').prop('checked',true);
